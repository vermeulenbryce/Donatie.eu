...
import {
  createCollectantRequest,
  fetchMyCollectantRequest,
  type CollectantRequestRow,
...
export function CollectantApplyPage() {
  const { shell } = useLegacyUiSession()
  const [existing, setExisting] = useState<CollectantRequestRow | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const row = await fetchMyCollectantRequest(shell.user.id)
      setExisting(row)
      setErr(null)
    } catch (e) {
  ...
  if (!shell?.user?.id) {
    return (
      <main className="page container" style={{ padding: '32px 0 64px', maxWidth: 560 }}>
        <h1 className="dash-title">Collectant worden</h1>
  ...
  if (existing && existing.status !== 'rejected') {
    return (
      <main className="page container" style={{ padding: '32px 0 64px', maxWidth: 560 }}>
        <h1 className="dash-title">Collectantstatus</h1>
        <div
          style={{
        ...
        >
          <strong style={{ fontSize: '1rem' }}>
            {existing.status === 'approved'
              ? 'Je bent goedgekeurd als collectant. 🎉'
  ...
  return (
    <main className="page container" style={{ padding: '32px 0 64px', maxWidth: 560 }}>
      <h1 className="dash-title">Word collectant</h1>
      <p style={{ color: '#6b7280', fontSize: '.92rem' }}>
        Help lokaal bij collectes en deur-aan deur contact. Vul het formulier in; de beheerder beoordeelt je aanmelding.
      </p>
      ...
      <label style={{ display: 'block', marginTop: 16 }}>
        ...
        <textarea
          ...
          placeholder="Waarom wil je collectant worden?"
