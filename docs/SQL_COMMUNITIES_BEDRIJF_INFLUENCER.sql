...
-- --------------------------------------------------
-- Donations: kolommen voor community-flow (vóór functies op public.donations)
-- --------------------------------------------------
...
begin
  ...
  -- Geen profielrij: vul aan vanuit auth.users, daarna opnieuw lezen
  if not found then
  ...
  -- Normaliseer profieltype
  v_at := lower(trim(coalesce(v_at, '')));
  if v_at in ('', 'individual') then
    v_at := 'individu';
  end if;

  -- Gebruik auth-metadata alleen als reparatiehint (niet blind overschrijven)
  ...
  -- Profiel ongeldig/leeg -> vul vanuit metadata
  ...
  -- Profiel zegt bedrijf/influencer maar metadata zegt individu én gebruiker is geen owner -> herstel naar individu
  ...
  -- Alleen bedrijf/influencer-accounts mogen niet als lid joinen
  ...
  -- Profiel zegt particulier maar jij bent eigenaar van deze community (data liep uiteen): herstel account_type
  ...
  -- Eerst lidmaatschap (eigenaar staat ook in community_members met rol owner)
  ...
  -- Eigenaar zonder lid-rij (legacy): aanvullen en als lid melden
...
-- Auto-community bij registratie uitschakelen: user kiest zelf wanneer hij deze aanmaakt
drop trigger if exists trg_profiles_auto_community on public.profiles;
drop trigger if exists trg_profiles_auto_community_update on public.profiles;
